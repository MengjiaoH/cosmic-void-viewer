    ...
    const ospcommon::math::vec2f &p)
{
 ...
 ospcommon::math::vec2f ArcballCamera::worldToPixel(ospcommon::math::vec3f worldPos, ospcommon::math::vec2i imgSize)
 {
  ...
  ospcommon::math::vec3f v = worldPos - pos;
  ...
  ospcommon::math::vec2f imgPlaneSize;
  float fovy = 60.f;
  float aspect = imgSize.x / (float)imgSize.y;
  imgPlaneSize.y = 2.f * tanf(ospcommon::math::deg2rad(0.5f * fovy));
  ...
  ospcommon::math::vec2f sp = ospcommon::math::vec2f(dot(screenDir, normalize(V) ), dot(screenDir, normalize(U))) / imgPlaneSize;
  const float depth = ospcommon::math::sign(t) * length(v);
